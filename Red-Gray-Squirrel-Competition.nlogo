globals
[ max-red-squirrels  ; don't let the red squirrel population grow too large
  max-grey-squirrels ; don't let the gray squirrel population grow too large
  %infected
]


; Two species of squirrel are both breeds of turtles
breed [ red-squirrels red-squirrel ]
breed [ grey-squirrels gray-squirrel ]

turtles-own
[
  energy    ; both species of squirrels have energy
  infected? ; If true, the squirrel is infected
]
...
to setup
  clear-all
  ifelse netlogo-web? [ set max-red-squirrels 1000 ] [ set max-red-squirrels 3000 ]
  ifelse netlogo-web? [ set max-grey-squirrels 1000 ] [ set max-grey-squirrels 3000 ]

  ; each walnuts' state of growth and growing logic need to be set up
  ifelse model-version = "squirrels-nuts" [
    ask patches [
      set pcolor one-of [ brown green ]
      ifelse pcolor = brown
        [ set countdown nuts-regrowth-time ]
      [ set countdown random nuts-regrowth-time ] ; initialize walnuts regrowth clocks randomly for green patches
    ]
  ]
  [
  ...
  create-red-squirrels initial-number-reds  ; create the red squirrels, then initialize their variables
  [
    set shape  "squirrel"
    set color red
    set size 1  ; easier to see
    set label-color red - 2
    set energy random (2 * reds-gain-from-food)
    setxy random-xcor random-ycor
    set infected? false
  ]

  create-grey-squirrels initial-number-grey  ; create the grey squirrels, then initialize their variables
  [
    set shape "squirrel"
    set color gray
    set size 1.5  ; easier to see
    set energy random (2 * grey-gain-from-food)
    setxy random-xcor random-ycor
    set infected? true
  ]

  let random-agent one-of grey-squirrels
  ask random-agent [ set infected? true ]

  set %infected (count turtles with [ color = yellow ] / count turtles) * 100
...
to go
  ; stop the model if there are no squirrels
  if not any? turtles [ stop ]
  ; stop the model if there are no squirrels gets very large
  if not any? grey-squirrels and count red-squirrels > max-red-squirrels [ user-message "Red squirrels can survive" stop ]
  ask red-squirrels [
    move

    ; in this version, squirrels eat walnuts, walnuts grows, and it costs squirrel energy to move
    if model-version = "squirrels-nuts" [
      set energy energy - 1  ; deduct energy for squirrels
      eat-nuts1              ; red squirrels eat walnuts
      death                  ; squirrels die from starvation

    ]
    reproduce-red  ; red squirrels reproduce at a random rate governed by a slider
    if infected? [
      spread-infection ; infection rules
      die-for-virus
    ]
  ]

  if not any? red-squirrels and count grey-squirrels > max-grey-squirrels [ user-message "Red squirrels are extinct" stop ]
  ask grey-squirrels [
    move
    if model-version = "squirrels-nuts" [
      set energy energy - 1  ; grey squirrels lose energy as they move
      eat-nuts2              ; grey squirrels eat walnuts
      death                  ; grey squirrels die if they run out of energy
    ]
    reproduce-gray ; grey squirrels reproduce at a random rate governed by a slider
    if infected? [
      spread-infection ; infection rules
    ]
  ]
  ...
  if model-version = "squirrels-nuts" [ ask patches [ grow-nuts ] ]
...
to eat-nuts1  ;
  ; red squirrels eat walnuts and turn the patch green
  if pcolor = brown [
    set pcolor green
    set energy energy + reds-gain-from-food  ; red squirrels gain energy by eating
  ]
end

to eat-nuts2  ;
  ; grey squirrels eat walnuts and turn the patch green
  if pcolor = brown [
    set pcolor green
    set energy energy + grey-gain-from-food  ; grey squirrels gain energy by eating
  ]
end

to reproduce-red  ; red squirrel procedure
  if random-float 100 < reds-reproduce [  ; throw "dice" to see if you will reproduce
    set energy (energy / 2)               ; divide energy between parent and offspring
    hatch 1 [ rt random-float 360 fd 1 ]  ; hatch an offspring and move it forward 1 step
  ]
end

to reproduce-gray  ; gray squirrel procedure
  if random-float 100 < grey-reproduce [  ; throw "dice" to see if you will reproduce
    set energy (energy / 2)               ; divide energy between parent and offspring
    hatch 1 [ rt random-float 360 fd 1 ]  ; hatch an offspring and move it forward 1 step
  ]
end

to death  ; red squirrels procedure
...
to spread-infection
  if infected? = true
    [ ask other turtles-here in-radius 1 [ if random 100 < %infectiousness
      [ set color yellow ]]
    ]
end
...
to grow-nuts  ; patch procedure
  ; countdown on green patches: if you reach 0, grow some nuts
  if pcolor = green [
    ifelse countdown <= 0
      [ set pcolor brown
        set countdown nuts-regrowth-time ]
...
to display-labels
  ...
  if show-energy? [
    ask grey-squirrels [ set label round energy ]
    if model-version = "squirrels-nuts" [ ask red-squirrels [ set label round energy ] ]
  ]
end
...
"" ""
PENS
"red" 1.0 0 -2674135 true "" "plot count red-squirrels"
"gray" 1.0 0 -12895429 true "" "plot count grey-squirrels"
"nuts / 4" 1.0 0 -5207188 true "" "if model-version = \"squirrels-nuts\" [ plot count nuts / 4 ]"
"infect" 1.0 0 -1184463 true "" "plot %infected"
...
@#$#@#$#@
<experiments>
  <experiment name="1° experiment" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>count grey-squirrels</metric>
    <metric>count red-squirrels</metric>
    ...
    <enumeratedValueSet variable="grey-reproduce">
      <value value="37"/>
      <value value="67"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reds-gain-from-food">
      <value value="25"/>
      <value value="50"/>
      <value value="75"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="grey-gain-from-food">
      <value value="25"/>
      <value value="50"/>
      <value value="75"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial-number-reds">
      <value value="10"/>
      <value value="25"/>
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="model-version">
      <value value="&quot;squirrels-nuts&quot;"/>
    </enumeratedValueSet>
    <steppedValueSet variable="nuts-regrowth-time" first="25" step="50" last="75"/>
    <enumeratedValueSet variable="initial-number-grey">
      <value value="10"/>
      <value value="25"/>
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reds-reproduce">
      <value value="37"/>
      <value value="67"/>
    </enumeratedValueSet>
  </experiment>
